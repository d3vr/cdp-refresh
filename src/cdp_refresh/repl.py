    ...
    def get_completions(self, document, complete_event):
        ...
        if not word:
            ...
            for cmd in sorted(self.commands.keys()):
                yield Completion(
                    cmd, start_position=0, display_meta=self.commands[cmd]["help"]
                )
            return
        ...
        for cmd in sorted(self.commands.keys()):
            if cmd.startswith(word):
                ...
                yield Completion(
                    cmd, start_position=-len(word), display_meta=display_meta
                )
...
class REPL:
    ...
    async def start(self) -> None:
        ...
        self.running = True
        console.print(
            "[bold cyan]CDP Refresh interactive mode - type [green]help[/] for available commands[/]"
        )
        ...
        while True:
            try:
                ...
                refresh_task = asyncio.create_task(self.refresh_event.wait())
                    ...
                    [prompt_task, refresh_task], return_when=asyncio.FIRST_COMPLETED
                )

                # Cancel any pending tasks
                for task in pending:
                    task.cancel()
                    # Wait a small amount of time for the cancellation to complete
                    await asyncio.sleep(0.1)

                # Check which task completed
                if prompt_task in done and not prompt_task.cancelled():
                    # User input was received
                    self.refresh_event.clear()  # Reset the event
                    try:
                        return await prompt_task
                    except asyncio.CancelledError:
                        # If it was cancelled after we checked
                        console.print("")
                        continue
                else:
                    ...
                    console.print("")
                    # Make sure to fully cancel the prompt task if it's running
                ...
                # Handle cancelled tasks more explicitly
                console.print("")
                continue
            except Exception as e:
    ...
    async def _prompt_user(self) -> str:
        ...
        try:
            ...
            return await loop.run_in_executor(
                None, lambda: console.input("[bold green]cdp-refresh>[/] ").strip()
            )
    ...
    async def handle_file_change(self, changed_files: list = None) -> None:
        """Handle file change event and set the refresh event.

        This method is called by the watcher when files change.

        Args:
            changed_files: List of changed file paths relative to the watched directory
        """
        if changed_files:
            ...
            console.print(
                f"[yellow]Files changed: [bold]{files_str}[/], reloading page...[/]"
            )
        else:
        ...
        try:
            await self.client.reload_page()
            console.print("[green]Page reloaded successfully[/]")
        except Exception as e:
            console.print(f"[red]Error reloading page: {e}[/]")
        finally:
            # Always signal that a refresh has occurred to return to prompt
            self.refresh_event.set()
            # Ensure we resume input promptly
            await asyncio.sleep(0.1)
    ...
    async def _cmd_select_tab(self) -> None:
        ...
        try:
            ...
            # Import the tab selection function from cli module
            from cdp_refresh.cli import _select_tab

            # Use the interactive arrow-key tab selection method from CLI
            ...
            if tab:
                ...
                console.print(
                    f"[bold green]Now refreshing tab:[/] [cyan]{tab.title}[/]"
                )
            else:
                ...
                if hasattr(self.client, "current_tab") and self.client.current_tab:
                    ...
                    await self.client.connect(
                        previous_tab.websocket_url, tab=previous_tab
                    )
                    console.print(
                        f"[yellow]Reconnected to previous tab:[/] [cyan]{previous_tab.title}[/]"
                    )
        except Exception as e:
            if DEBUG_MODE:
            ...
            try:
                if hasattr(self.client, "current_tab") and self.client.current_tab:
                    ...
                    console.print("[yellow]Reconnected to previous tab.[/]")
